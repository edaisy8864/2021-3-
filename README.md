...
|REQ|Type|Priority|Content|
|:---:|:---:|:---:|:---|
|173|FR|1|시스템은 주문이 들어온 테이블 번호, 주문 내용을 보여줘야 한다.|
|174|FR|1| 시스템은 처음 주문이 들어왔을 때, 예상 대기 시간을 0분 0초로 초기화해야 한다.|
|175|FR|1|시스템은 Admin이 예상 대기 시간을 +,- 버튼을 이용하여 설정할 수 있게 해야 한다.|
|176|FR|1|시스템은 Admin이 "조리시작"버튼을 눌렀을 시 해당 주문의 Customer에게 "조리중"메세지를 보내줘야 한다.|
...
|179|FR|1|시스템은 Admin이 "조리완료"버튼을 눌렀을 시 해당 주문의 Customer에게 "조리완료" 메세지를 보내줘야 한다.|
|180|FR|3|시스템은 Admin이 "조리완료"버튼을 눌렀을 시 해당 주문 주문현황 목록에서 제거해야 한다.|
