...
|163|FR|4|시스템은 Customer가 후기를 작성할 수 있도록 "주문완료" 탭에서 항목의 상태가 “조리완료" 일 때, 해당 항목의 "후기쓰기" 버튼을 활성화 시켜야 한다.|
|163|FR|4|시스템은 "후기쓰기" 버튼을 마우스 클릭을 사용하여 선택했을 때. <후기쓰기> 페이지로 이동해야 한다.|
|164|FR|4|시스템은 Customer가 로그인한 상태일 때, "주문완료" 탭의 모든 항목이 "조리완료" 상태가 되면 총 주문금액이 "Admin"이 설정한 조건에 만족하는 경우 해당 Customer 계정의 포인트를 적립하여 데이터베이스에 저장하고, 보안을 위해 Customer 계정을 10분 후 자동 로그아웃시켜 <Customer 모드> 화면으로 이동해야 한다.|
|165|NFR|1|시스템은 Customer가 <주문하기> 화면으로 이동했을 때 "메인" 탭, "사이드" 탭, "음료 및 디저트" 탭, "서비스" 탭, "공지사항" 탭, "주문 완료" 탭 “후기” 탭 등 7가지의 탭을 화면 상단에 보여주고, "메인" 탭의 내용을 초기 화면에 디스플레이 한다.|
...
|168|NFR|1| 시스템은 “후기” 탭에서 Customer들이 등록한 후기들을 5개씩 최신순으로 보여주고 “저번 주 가장 많이 주문한 음식”과 “저번 주 가장 별점이 높은 음식”을 데이터베이스 분석을 통해 보여주어야 한다.|
...
|REQ|Type|Priority|Content|
|:---:|:---:|:---:|:---|
|148|FR|4|시스템은 Customer가 "별점" 버튼을 마우스로 선택하여 0개~5개의 별점을 선택할 수 있게 해야한다.|
|148|FR|4|시스템은 <후기쓰기>의 초기 설정 별점이 5개가 되게 해야한다.|
|148|FR|4|시스템은 Customer가 "한줄평" 텍스트상자를 마우스로 선택하고 키보드를 사용하여 후기를 작성할 수 있게 있게 해야한다.|
|148|FR|4|시스템은 Customer가 "취소" 버튼을 마우스로 선택하면 <주문하기>화면의 "주문완료" 탭으로 이동해야한다.|
|148|FR|4|시스템은 Customer가 "후기등록" 버튼을 마우스로 선택하면 별점과 한줄평의 내용을 데이터베이스에 저장하고 해당 내용을 <주문하기>화면의 "후기" 탭에 업데이트 한뒤 "후기" 탭으로 이동해야한다.|
|165|NFR|1|시스템은 Customer가 <후기쓰기> 화면으로 이동했을 때 Customer가 선택한 조리완료 항목의 정보, "별점" 버튼, "한줄평" 텍스트상자, "취소" 버튼, "후기등록" 버튼을 보여주어야 한다.
