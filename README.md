...
|174|FR| | 시스템은 처음 주문이 들어왔을 때, 예상 대기 시간을 0분 0초로 초기화해야 한다.|
|175|FR| |시스템은 Admin이 예상 대기 시간을 +,- 버튼을 이용하여 설정할 수 있게 해야 한다.|
|176|FR| |시스템은 Admin이 "조리시작"버튼을 눌렀을 시 해당 주문의 Customer에게 "조리중"메세지를 보내줘야 한다.|
|177|FR| |시스템은 Admin이 "조리시작"버튼을 눌렀을 시 해당 주문의 타이머를 작동시켜야 한다.|
|178|FR| |시스템은 Admin이 "조리시작"버튼을 눌렀을 시 타이머의 시간 조작 버튼을 Admin이 선택할 수 없게 비활성화해야 한다.|
|179|FR| |시스템은 Admin이 "조리완료"버튼을 눌렀을 시 해당 주문의 Customer에게 "조리완료" 메세지를 보내줘야 한다.|
