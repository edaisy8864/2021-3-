...
|44|NFR|1|시스템은 Admin이 입력한 이메일이 이메일 형식에 부합하지 않으면 해당 내용을 알리는 경고 메시지 창을 띄워야 한다. |
|45|NFR|1|시스템은 Admin이 입력한 이메일이 데이터베이스에 존재하지 않으면 해당 내용을 알리는 경고 메시지 창을 띄워야 한다. |
|46|NFR|1|시스템은 Admin이 입력한 아이디가 데이터베이스에 존재하지 않으면 해당 내용을 알리는 경고 메시지 창을 띄워야 한다. |
|47|NFR|1|시스템은 Admin이 입력한 아이디와 이메일이 일치하는 계정이 데이터베이스에 존재하지 않으면 해당 내용을 알리는 경고 메시지 창을 띄워야 한다. |

|REQ|Type|Priority|Content|
|:---:|:---:|:---:|:---|
|32|FR|3|시스템은 Admin이 화면을 터치하여 “ID찾기” 판넬에 있는 “확인” 버튼을 클릭하였을 때 Admin이 입력한 텍스트 상자에 공백이 있는지 확인해야 한다. |
|33|FR|3|시스템은 Admin이 입력한 텍스트 상자에 공백이 없을 경우 Admin이 입력한 이메일이 이메일 형식에 부합하는지 확인해야 한다. |
|34|FR|3|시스템은 Admin이 입력한 이메일이 이메일 형식에 부합하는 것을 확인했을 때, Admin이 입력한 이메일이 데이터베이스에 존재하는지 확인해야 한다. |
|35|FR|4|시스템은 Admin이 입력한 이메일이 데이터베이스에 존재하는지 확인했을 때, Admin이 입력한 이메일 계정의 아이디를 Admin의 이메일 주소로 메일을 송신해야 한다. |
|36|FR|4|시스템은 Admin이 입력한 이메일로 아이디가 적힌 메일을 송신 후 화면에 이메일로 아이디를 송신했다는 문구와 같이 “로그인 페이지로”버튼과 “PW찾기”버튼이 있는 메시지 창을 띄워야 한다.|
|37|FR|4|시스템은 Admin이 화면을 터치하여 메시지 창에 있는 “로그인 페이지로”버튼과 “PW찾기”버튼을 클릭할 수 있도록 허용해야 한다.|
|38|FR|3|시스템은 Admin이 화면을 터치하여 메시지 창에 있는 “로그인 페이지로”버튼을 클릭했을 경우 <Admin Login>화면으로 이동해야 한다. |
|39|FR|3|시스템은 Admin이 화면을 터치하여 메시지 창에 있는 “PW찾기”버튼을 클릭했을 경우 다시 <Find Admin ID&PW>화면으로 이동해야 한다. |
  
|REQ|Type|Priority|Content|
|:---:|:---:|:---:|:---|
|40|FR|3|시스템은 Admin이 화면을 터치하여 “PW찾기” 판넬에 있는 “확인”버튼을 클릭했을 경우 Admin이 입력한 텍스트 상자들에 공백이 있는지 확인해야한다. |
|41|FR|3|시스템은 텍스트 상자에 공백이 없는 것을 확인했을 경우 Admin이 입력한 이메일이 이메일 형식에 부합하는지 확인해야 한다. |
...
|REQ|Type|Priority|Content|
|:---:|:---:|:---:|:---|
|173|FR|4|시스템은 주문이 들어온 테이블 번호, 주문 내용을 보여줘야 한다.|
|174|FR|1|시스템은 처음 주문이 들어왔을 때, 예상 대기 시간을 0분 0초로 초기화해야 한다.|
|175|FR|1|시스템은 Admin이 예상 대기 시간을 +,- 버튼을 이용하여 설정할 수 있게 해야 한다.|
|176|FR|3|시스템은 Admin이 "조리시작"버튼을 눌렀을 시 해당 주문의 Customer에게 "조리중"메세지를 보내줘야 한다.|
|177|FR|2|시스템은 Admin이 "조리시작"버튼을 눌렀을 시 해당 주문의 타이머를 작동시켜야 한다.|
|178|FR|1|시스템은 Admin이 "조리시작"버튼을 눌렀을 시 타이머의 시간 조작 버튼을 Admin이 선택할 수 없게 비활성화해야 한다.|
|179|FR|3|시스템은 Admin이 "조리완료"버튼을 눌렀을 시 해당 주문의 Customer에게 "조리완료" 메세지를 보내줘야 한다.|
|180|FR|4|시스템은 Admin이 "조리완료"버튼을 눌렀을 시 해당 주문 주문현황 목록에서 제거해야 한다.|
|181|FR|4|시스템은 주문이 "조리중"이 아니고 Customer가 "주문취소" 메세지를 보냈을 경우, 주문현황 목록에서 제거해야 한다.|
|182|NFR|2|시스템은 각 주문이 병렬적으로 실행되게 해야 한다.|
...
|73|FR|4|
|74|FR|4|
|75|FR|4|시스템은 <Customer 모드> 화면에서 Customer가 마우스의 클릭을 사용하여 “비회원” 버튼, “회원”버튼을 선택할 수 있게 허용해야 한다.|
|76|FR|4|시스템은 Customer가 "비회원" 버튼을 선택했을 때 <주문하기> 화면으로 이동해야 한다.|
|77|FR|4|시스템은 Customer가 "회원" 버튼을 선택했을 때 <Customer 로그인> 화면으로 이동해야 한다.|
|78|FR|4|
|79|NFR|5|시스템은 <Customer 모드> 화면으로 이동했을 때 "회원" 버튼과 "비회원" 버튼을 화면에 보여주어야 한다.|
...
|166|FR|4|시스템은 Customer가 로그인한 상태일 때, "주문완료" 탭의 모든 항목이 "조리완료" 상태가 되면 총 주문금액이 "Admin"이 설정한 조건에 만족하는 경우 해당 Customer 계정의 포인트를 적립하여 데이터베이스에 저장하고, 보안을 위해 Customer 계정을 10분 후 자동 로그아웃시켜 <Customer 모드> 화면으로 이동해야 한다.|
...
|178|NFR|1|시스템은 Customer가 "결제하기" 버튼을 선택했을 때 Admin에게 알람이 가게 해야한다.|
...
|REQ|Type|Priority|Content|
|:---:|:---:|:---:|:---|
|176|FR|4|시스템은 Customer가 "별점" 버튼을 마우스로 선택하여 0개~5개의 별점을 선택할 수 있게 해야한다.|
|177|FR|4|시스템은 <후기쓰기>의 초기 설정 별점이 5개가 되게 해야한다.|
|178|FR|4|시스템은 Customer가 "한줄평" 텍스트상자를 마우스로 선택하고 키보드를 사용하여 후기를 작성할 수 있게 있게 해야한다.|
|179|FR|4|시스템은 Customer가 "취소" 버튼을 마우스로 선택하면 <주문하기>화면의 "주문완료" 탭으로 이동해야한다.|
|180|FR|4|시스템은 Customer가 "후기등록" 버튼을 마우스로 선택하면 별점과 한줄평의 내용을 데이터베이스에 저장하고 해당 내용을 <주문하기>화면의 "후기" 탭에 업데이트 한뒤 "후기" 탭으로 이동해야한다.|
|181|NFR|1|시스템은 Customer가 <후기쓰기> 화면으로 이동했을 때 Customer가 선택한 조리완료 항목의 정보, "별점" 버튼, "한줄평" 텍스트상자, "취소" 버튼, "후기등록" 버튼을 보여주어야 한다.
