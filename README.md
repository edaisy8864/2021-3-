...
|149|FR|4|시스템은 시스템은 Custoemr가 "메인으로" 버튼을 선택했을 때 회원일 경우 로그아웃 진행 후에 <Customer 모드> 화면으로 되돌아가게 해야한다.
...
|178|NFR|1|시스템은 "손님" 버튼을 마우스로 클릭한 경우 <Customer 로그인> 화면으로 이동해야 한다.|
|179|NFR|1|시스템은 UI 형태가 직관적이고 쉬워서 누구라도 사용할 수 있어야 한다.|
